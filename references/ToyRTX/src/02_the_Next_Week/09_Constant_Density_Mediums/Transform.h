...
namespace RTX
{
	...
	public:
		Translate(Hitable::CPtr object, const vec3& displacement)
	...
	public:
		RotateY(Hitable::CPtr object, float angle) :object(object) {
			auto radians = Define::degrees_to_radians(angle);
			sin_theta = sin(radians);
			cos_theta = cos(radians);
			hasbox = object->boundingBox(0, 1, bbox);

			vec3 min(Define::infinity);
			vec3 max(-Define::infinity);
