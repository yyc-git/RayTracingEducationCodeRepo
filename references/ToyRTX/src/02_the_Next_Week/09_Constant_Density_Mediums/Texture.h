...
namespace RTX {
	...
	public:
		...
		virtual rgb value(const vec2& uv, const vec3& p) const override {
			return rgb(1.0f) * 0.5f * (1.0f + sin(scale * p.z + 10.0f * noise.turb(p)));
		}
	...
	public:
		...
		virtual rgb value(const vec2& uv, const vec3& p) const override {
			...
			if (!img.IsValid())
				return rgb(0.0f, 1.0f, 1.0f);
			// Clamp input texture coordinates to [0,1] x [1,0]
			auto u = clamp(uv.x, 0.0f, 1.0f);
			auto v = 1.0f - clamp(uv.y, 0.0f, 1.0f);  // Flip V to image coordinates
			...
			return rgb(scale * pixel[0], scale * pixel[1], scale * pixel[2]);
		}
