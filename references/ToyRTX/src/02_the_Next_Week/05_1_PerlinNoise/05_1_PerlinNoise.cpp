...
Hittable_list twoSpheres(float t0, float t1)
{
	Hittable_list world;
	auto checker = ToPtr(new CheckerTexture(rgb(0.2, 0.3, 0.1), rgb(0.9, 0.9, 0.9)));
	world.add(ToPtr(new Sphere(vec3(0.0f, -8.01f, 0.0f), 8.0f, ToPtr(new Lambertian(checker)))));
    auto pertext = ToPtr(new NoiseTexture());
	world.add(ToPtr(new Sphere(vec3(0.0f, 8.01f, 0.0f), 8.0f, ToPtr(new Lambertian(pertext)))));
