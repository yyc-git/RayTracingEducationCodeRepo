...
Hittable_list creatWorld()
{
    ...
    world.add(ToPtr(new Sphere(vec3(-2.0f, 1.0f, 0.0f), 1.0f, ToPtr(new Lambertian(0.4f, 0.2f, 0.1f)))));
    world.add(ToPtr(new Sphere(vec3(-6.0f, 1.0f, 0.0f), 1.0f, ToPtr(new Refraction(2.5f, 0.0f, 0.98f, 0.98f)))));
    world.add(ToPtr(new Sphere(vec3(0.0f, -300.5f, 0.0f), 300.0f, ToPtr(new Lambertian(0.5f, 0.5f, 0.5f)))));
...
int main(int argc, char ** argv) {
    ...
    //World
    Hittable_list world(std::move(creatWorld( )));
    ...
    Camera::Ptr camera = ToPtr(new Camera(lookfrom, lookat,
        vup, 25.0f, aspect_ratio, dist_to_focus, aperture));
/*
*下面提供了两种方式供选择
*第一种是每个loop一次更新一次图片，loop N轮
*第二种是一个像素点直接loop N轮，逐像素计算
*从道理上来讲都是N次采样，应该结果是相同的，但是本人测试后发现，
*在高性能的台机上，第一种方式在loop很大以后会出现边界糊的感觉，
*目前具体原因不太清楚，本人猜测，可能是随机数出现了一些问题，
*如果有人知道原因，请联系本人，非常感谢
*不过目前看来50次loop两种方式似乎差距并不是很显著。
*/
